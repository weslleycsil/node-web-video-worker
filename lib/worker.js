...
proto.process = function process (job, done) {
  ...
  worker.log2.info('[JOB]', job.id, 'Processing')

  // Create the ffmpeg object from input data
  worker.log.info('[JOB]', job.id, 'Input', job.data.input)
  ...
  function gotInput (error, ffmpeg) {
    ...
    worker.log2.info('[JOB]', job.id, 'Preset', job.data.preset)
    ...
    // Do output
    worker.log2.info('[JOB]', job.id, 'Output', job.data.output)
    worker.output(job.data.output.type)(job, ffmpeg, doneOutput)

    // Echo command
    var args = ffmpeg._getArguments()
    worker.log2.info('[JOB]', job.id, 'Command:', 'ffmpeg', args)
  }
  ...
  function doneOutput (err) {
    ...
    worker.log2.info('[JOB]', job.id, 'Complete')
    done()
  }
}
