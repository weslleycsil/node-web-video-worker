...
proto.process = function process (job, done) {
  ...
  worker.log.info('[JOB]', job.id, 'Processing')
  ...
  function gotInput (error, ffmpeg) {
    ...
    worker.log.info('[JOB]', job.id, 'Preset', job.data.preset)
    ...
    // Do output
    worker.log.info('[JOB]', job.id, 'Output', job.data.output)
    worker.output(job.data.output.type)(job, ffmpeg, doneOutput)

    // Echo command
    var args = ffmpeg._getArguments()
    worker.log.info('[JOB]', job.id, 'Command:', 'ffmpeg', args)
  }
  ...
  function doneOutput (err) {
    ...
    worker.log.info('[JOB]', job.id, 'Complete')
    done()
  }
}
